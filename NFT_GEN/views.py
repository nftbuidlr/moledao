...
from NFT_GEN.models import Image,LayersModel, ProjectDesc
...
def LayerGet(request):
    ...
    tuser=User.objects.get(username=mainuser)
    ...
    if request.method == 'POST':
        fm = ProjRegistration(request.POST)
        #fm.user = tuser
        if fm.is_valid():
            fm.save()
            fm = ProjRegistration()
        ...
        fm = ProjRegistration(instance=tuser)
    """
...
def LayerDel(request, pk):
	...
	context={'layoutDataV':layoutDataF}

	return render(request, 'layerdel.html', context)
...
def add_proj(request):
    ...
    if request.method == 'POST':
        ...
        fm = ProjRegistration()
    return render(request,'layout.html',{'form':fm,'layoutDataV':layoutDataF,'imagesObjects':imagesObjects})
"""
...
"""
imgs = []
for f in sorted(glob.glob(img_path+'/*.png')):
imgs.append(PIL.Image.open(f))
print(sorted(glob.glob(img_path)))
print(img)
print('-----------------------------')
print(*imgs)
